...
// informação no PC
uint16_t ZP0(CPU_6502 *cpu){
     uint16_t addrs_abs;
     addrs_abs = bus_read(cpu->PC);
     cpu->PC++;
...
// array
uint16_t ZPX(CPU_6502 *cpu){
    uint16_t addrs;
    addrs = bus_read(cpu->PC + cpu->X);
    cpu->PC++;
...
uint16_t ZPY(CPU_6502 *cpu){
    uint16_t addrs;
    addrs = bus_read(cpu->PC + cpu->Y);
    cpu->PC++;
...
// É usado em funções de brench
uint16_t REL(CPU_6502 *cpu){
    uint16_t addrs;
    addrs = bus_read(cpu->PC);
    cpu->PC++;
...
uint16_t ABX(CPU_6502 *cpu){
    uint16_t lo = bus_read(cpu->PC);
    cpu->PC++;
    uint16_t hi = bus_read(cpu->PC);
    cpu->PC++;

    uint16_t addrs = (hi << 8) | lo;
    addrs += cpu->X;
...
// interessa
uint16_t IND(CPU_6502 *cpu){
	uint16_t ptr_lo = bus_read(cpu->PC);
	cpu->PC++;
	uint16_t ptr_hi = bus_read(cpu->PC);
	cpu->PC++;
	...
	{
		cpu->addrs = (bus_read(ptr & 0xFF00) << 8) | bus_read(ptr + 0);
	}
	else // Behave normally
	{
		cpu->addrs = (bus_read(ptr + 1) << 8) | bus_read(ptr + 0);
	}
...
uint16_t IZX(CPU_6502 *cpu){
    uint16_t t = bus_read(cpu->PC);
	cpu->PC++;

	uint16_t lo = bus_read((t + cpu->X) & 0x00FF);
	uint16_t hi = bus_read((t + cpu->X + 1) & 0x00FF);
...
uint16_t IZY(CPU_6502 *cpu){
    uint16_t t = bus_read(cpu->PC);
	cpu->PC++;

	uint16_t lo = bus_read((t + cpu->Y) & 0x00FF);
	uint16_t hi = bus_read((t + cpu->Y + 1) & 0x00FF);
...
uint8_t ADC(CPU_6502 *my_cpu){
    uint16_t temp = fetch(my_cpu->addrs);

    temp += my_cpu->AC + GetFlag(C, my_cpu);

    SetFlag(C, my_cpu, temp > 255);

    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0);

    SetFlag(V, my_cpu, ( (~((uint16_t)my_cpu->AC ^ (uint16_t)fetch(my_cpu->addrs)) & ( (uint16_t)my_cpu->AC ^ temp) & 0x0080) ));

    SetFlag(N, my_cpu, temp & 0x80);

    my_cpu->AC = temp & 0x00FF;
...
uint8_t AND(CPU_6502 *my_cpu){
    my_cpu->AC = my_cpu->AC & fetch(my_cpu->addrs);

    SetFlag(Z, my_cpu, my_cpu->AC == 0x00);
    SetFlag(N, my_cpu, my_cpu->AC & 0x80);
...
uint8_t ASL(CPU_6502 *my_cpu){
    uint16_t temp = fetch(my_cpu->addrs) << 1;

    SetFlag(C, my_cpu, (temp & 0xFF00) > 0);
    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0x00);
    SetFlag(N, my_cpu,temp & 0x80);
    ...
    if(my_cpu->addrs == 0){
        ...
        bus_write(my_cpu->addrs, temp & 0x00FF);
    }
...
uint8_t BCC(CPU_6502 *my_cpu){
    if(GetFlag(C, my_cpu) == 0){
        my_cpu->clk++;

        my_cpu->addrs += my_cpu->PC;

        if((my_cpu->addrs & 0xFF00) != (my_cpu->PC & 0xFF00)){
            my_cpu->clk++;
        }
        my_cpu->PC = my_cpu->addrs;
    }
    return 0;
}
...
uint8_t BCS(CPU_6502 *my_cpu){
    if(GetFlag(C, my_cpu) == 1){
        my_cpu->clk++;
...
uint8_t BEQ(CPU_6502 *my_cpu){
    if(GetFlag(Z, my_cpu) == 1){
        my_cpu->clk++;
...
uint8_t BIT(CPU_6502 *my_cpu){
    uint16_t temp = my_cpu->AC & fetch(my_cpu->addrs);
    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0x00);
    SetFlag(N, my_cpu, fetch(my_cpu->addrs) & (1 << 7));
    SetFlag(V, my_cpu, fetch(my_cpu->addrs) & (1 << 6));
...
uint8_t BMI(CPU_6502 *my_cpu){
    if(GetFlag(N, my_cpu) == 1){
        my_cpu->clk++;
...
uint8_t BNE(CPU_6502 *my_cpu){
    if(GetFlag(Z, my_cpu) == 0){
        my_cpu->clk++;
...
uint8_t BPL(CPU_6502 *my_cpu){
    if(GetFlag(N, my_cpu) == 0){
        my_cpu->clk++;
...
//      --------------------------------------------
//      implied       BRK           00    1     7
uint8_t BRK(CPU_6502 *my_cpu){
    my_cpu->PC++;

    SetFlag(I, my_cpu, 1);
    bus_write(0x0100 + my_cpu->SP, (my_cpu->PC >> 8) & 0x00FF);
    my_cpu->SP--;
    bus_write(0x0100 + my_cpu->SP, my_cpu->PC & 0x00FF);
    my_cpu->SP--;
    ...
    my_cpu->PC = ((uint16_t)bus_read(0xFFFE) | (uint16_t)bus_read(0xFFFF) << 8);
    return 0;
}
...
uint8_t BVC(CPU_6502 *my_cpu){
    if(GetFlag(V, my_cpu) == 0){
        my_cpu->clk++;
...
uint8_t BVS(CPU_6502 *my_cpu){
    if(GetFlag(V, my_cpu) == 1){
        my_cpu->clk++;
...
uint8_t CMP(CPU_6502 *my_cpu){
    uint16_t temp = (uint16_t)my_cpu->AC - (uint16_t)fetch(my_cpu->addrs);
    SetFlag(C, my_cpu, my_cpu->AC >= fetch(my_cpu->addrs));
    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0x0000);
    SetFlag(N, my_cpu, temp & 0x0080);
    return 1;
...
uint8_t CPX(CPU_6502 *my_cpu){
    uint16_t temp = (uint16_t)my_cpu->X - (uint16_t)fetch(my_cpu->addrs);
    SetFlag(C, my_cpu, my_cpu->X >= fetch(my_cpu->addrs));
    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0x0000);
    SetFlag(N, my_cpu, temp & 0x0080);
    return 0;
}
...
uint8_t CPY(CPU_6502 *my_cpu){
    uint16_t temp = (uint16_t)my_cpu->Y - (uint16_t)fetch(my_cpu->addrs);
    SetFlag(C, my_cpu, my_cpu->Y >= fetch(my_cpu->addrs));
    SetFlag(Z, my_cpu, (temp & 0x00FF) == 0x0000);
    SetFlag(N, my_cpu, temp & 0x0080);
    return 0;
}
...
//      --------------------------------------------
//      implied       DEC           CA    1     2
uint8_t DEX(CPU_6502 *my_cpu){
    my_cpu->X--;
    SetFlag(Z, my_cpu, my_cpu->X == 0x00);
    SetFlag(N, my_cpu, my_cpu->X == 0x80);
    return 0;
}
...
//      --------------------------------------------
//      implied       DEC           88    1     2
uint8_t DEY(CPU_6502 *my_cpu){
    my_cpu->Y--;
    SetFlag(Z, my_cpu, my_cpu->Y == 0x00);
    SetFlag(N, my_cpu, my_cpu->Y == 0x80);
    return 0;
}
...
uint8_t INC(CPU_6502 *my_cpu){
    uint8_t temp = fetch(my_cpu->addrs) + 1;
    bus_write(my_cpu->addrs, temp);
    SetFlag(Z, my_cpu, temp == 0x00);
    SetFlag(N, my_cpu, temp & 0x80);
    return 0;
}
...
uint8_t LDA(CPU_6502 *my_cpu){
    my_cpu->AC = fetch(my_cpu->addrs);
    SetFlag(Z, my_cpu, my_cpu->AC == 0x00);
    SetFlag(N, my_cpu, my_cpu->AC & 0x80);
    return 1;
}
...
uint8_t LDX(CPU_6502 *my_cpu){
    my_cpu->X = fetch(my_cpu->addrs);
    SetFlag(Z, my_cpu, my_cpu->X == 0x00);
    SetFlag(N, my_cpu, my_cpu->X & 0x80);
    return 1;
}
...
uint8_t LDY(CPU_6502 *my_cpu){
    my_cpu->Y = fetch(my_cpu->addrs);
    SetFlag(Z, my_cpu, my_cpu->Y == 0x00);
    SetFlag(N, my_cpu, my_cpu->Y & 0x80);
    return 1;
}
...
uint8_t LSR(CPU_6502 *my_cpu){
    uint8_t temp = fetch(my_cpu->addrs);

    SetFlag(C, my_cpu, temp & 0x01);
    temp = temp >> 1;
    SetFlag(Z, my_cpu, (temp == 0x00));
    SetFlag(N, my_cpu, temp & 0x80);
...
//      --------------------------------------------
//      implied       NOP           EA    1     2
uint8_t NOP(CPU_6502 *my_cpu){ return 0; }
...
uint8_t ORA(CPU_6502 *my_cpu){
    uint8_t temp = fetch(my_cpu->addrs);
    my_cpu->AC = my_cpu->AC | temp;
...
//      --------------------------------------------
//      implied       PHA           48    1     3
uint8_t PHA(CPU_6502 *my_cpu){
    bus_write(0x0100 + my_cpu->SP, my_cpu->AC);
    my_cpu->SP--;
    return 0;
}
...
//      --------------------------------------------
//      implied       PHP           08    1     3
uint8_t PHP(CPU_6502 *my_cpu){
    bus_write(0x0100 + my_cpu->SP, my_cpu->SR | B | _);
    SetFlag(B, my_cpu, 0);
    SetFlag(_, my_cpu, 0);
    return 0;
}
...
//      --------------------------------------------
//      implied       PLA           68    1     4
uint8_t PLA(CPU_6502 *my_cpu){
    my_cpu->SP++;
    my_cpu->AC = bus_read(0x0100 + my_cpu->SP);
    SetFlag(Z, my_cpu, my_cpu->AC == 0x00);
    SetFlag(N, my_cpu, my_cpu->AC & 0x80);
    return 0;
}
...
//      --------------------------------------------
//      implied       PLP           28    1     4
uint8_t PLP(CPU_6502 *my_cpu){
    my_cpu->SP++;
    my_cpu->SR = bus_read(0x0100 + my_cpu->SP);
    SetFlag(_, my_cpu, 1);
    return 0;
}
...
uint8_t ROL(CPU_6502 *my_cpu){
    uint8_t fetched = fetch(my_cpu->addrs);
    
    uint16_t temp = (uint16_t)(fetched << 1) | GetFlag(C, my_cpu);
    SetFlag(C, my_cpu, (temp & 0xFF00));
    SetFlag(N, my_cpu, (temp & 0x0080));
    SetFlag(Z, my_cpu, ((temp & 0x00FF) == 0x0000));

    // Agora preciso saber se vou salvar na memoria ou no acumulador
    if(my_cpu->myOpList[my_cpu->opcode].mF == IMP){
        my_cpu->AC = (temp & 0x00FF);
    }else{
        bus_write(my_cpu->addrs, (temp & 0x00FF));
    }
...
uint8_t ROR(CPU_6502 *my_cpu){
    uint8_t fetched = fetch(my_cpu->addrs);

    uint16_t temp = (uint16_t)(fetched >> 1) | (GetFlag(C, my_cpu) << 7);
    SetFlag(C, my_cpu, (temp & 0x0001));
    SetFlag(N, my_cpu, (temp & 0x0080));
    SetFlag(Z, my_cpu, ((temp & 0x00FF) == 0x0000));
...
uint8_t RTI(CPU_6502 *my_cpu){
    ...
    my_cpu->SP++;
    my_cpu->PC = (uint16_t)bus_read(0x0100 + my_cpu->SP);
    my_cpu->SP++;
    my_cpu->PC |= ((uint16_t)bus_read(0x0100 + my_cpu->SP) << 8);
...
uint8_t SBC(CPU_6502 *my_cpu){
    uint16_t temp = (uint16_t)my_cpu->AC + 1 + (uint16_t)GetFlag(C, my_cpu) + (uint16_t)(~fetch(my_cpu->addrs));

    my_cpu->AC = (temp & 0x00FF);
    
    SetFlag(N, my_cpu, (temp & 0x0080));
    SetFlag(C, my_cpu, (temp & 0x0100));
    SetFlag(Z, my_cpu, ((temp & 0x00FF) == 0x00));
    SetFlag(V, my_cpu, temp); // Ainda fazer
...
uint8_t STA(CPU_6502 *my_cpu){
    bus_write(my_cpu->addrs, my_cpu->AC);
    return 0;
}
...
uint8_t STX(CPU_6502 *my_cpu){
    bus_write(my_cpu->addrs, my_cpu->X);
    return 0;
}
...
uint8_t STY(CPU_6502 *my_cpu){
    bus_write(my_cpu->addrs, my_cpu->Y);
    return 0;
}
...
uint8_t XXX(CPU_6502 *my_cpu){ return 0; }
